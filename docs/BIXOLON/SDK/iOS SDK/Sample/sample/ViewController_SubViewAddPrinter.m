...
@interface ViewController_SubViewAddPrinter ()
{
}
@end

@implementation ViewController_SubViewAddPrinter
...
-(IBAction)onButton_OK:(UIButton*)sender
{
    ...
    if(_uiButtonSelectModel.titleLabel.text.length )
    {
        ...
        NSLog(@"printer : %@(%@)", newDevice.modelName, newDevice.address );
        
            if([_parentView respondsToSelector:@selector(addPrinter:)])
            {
                [_parentView addPrinter:newDevice];
            }
