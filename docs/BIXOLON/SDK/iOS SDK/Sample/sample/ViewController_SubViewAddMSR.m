...
@interface ViewController_SubViewAddMSR ()
{
}
@end

@implementation ViewController_SubViewAddMSR
...
-(IBAction)onButton_OK:(UIButton*)sender
{
    ...
    if(_uiButtonSelectModel.titleLabel.text.length )
    {
        ...
        UPOSMSR* newDevice = [[UPOSMSR alloc] init];
...
//        newDevice.selectedP
        ...
        newDevice.port         = _uiTextField_Port.text;//@"";
            ...
            if([_parentView respondsToSelector:@selector(addMSR:)])
            {
                [_parentView addMSR:newDevice];
            }
